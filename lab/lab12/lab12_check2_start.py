...
class MyApp(object):
    ...
    def draw(self):
        self.draw_lines(self.width/2, self.height/2, min(self.width, self.height)/4, 6)
    ...
    ## recursion ends!
    def draw_lines(self, centerx, centery, length, n):
        if n == 0:
            self.canvas.create_line(centerx, centery+length, centerx, centery-length, fill="black") 
            self.canvas.create_line(centerx+length, centery, centerx-length, centery, fill="black") 
            self.canvas.update()
            return 
        #self.canvas.after(10)
        self.canvas.create_line(centerx, centery+length, centerx, centery-length, fill="black") 
        self.canvas.create_line(centerx+length, centery, centerx-length, centery, fill="black") 
        self.draw_lines(centerx+length, centery, length/2, n-1)
        self.draw_lines(centerx-length, centery, length/2, n-1)
        self.draw_lines(centerx, centery+length, length/2, n-1)
        self.draw_lines(centerx, centery-length, length/2, n-1)
